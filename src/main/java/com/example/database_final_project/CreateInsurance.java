...
public class CreateInsurance {
    ...
    public static AnchorPane DriverPane() {
        Util.initializeThirdPane();
            ...
            Label driverIdlabel = Util.createLabel("Driver ID number", 485, 28, 110, 27);
            Label firstNameLabel = Util.createLabel("First name", 479, 75, 110, 27);
            Label secondNameLabel = Util.createLabel("Second name", 651, 75, 110, 27);
            Label thirdNameLabel = Util.createLabel("Third name", 479, 151, 110, 27);
            Label lastNameLabel = Util.createLabel("Last name", 651, 151, 110, 27);
            Label insuranceIdLabel = Util.createLabel("Insurance ID", 651, 222, 110, 27);
            Label dateOfBirthLabel = Util.createLabel("Date of Birth", 479, 222, 110, 27);
            Label drivaingImageLabel = Util.createLabel("Driving image", 478, 298, 110, 27);


            TextField driverIdField = Util.createTextField("Driver ID number", 650, 28, 145, 27);
            TextField firstNameField = Util.createTextField("First name", 479, 102, 145, 27);
            TextField secondNameField = Util.createTextField("Second name", 650, 102, 145, 27);
            TextField thirdNameField = Util.createTextField("Third name", 479, 178, 145, 27);
            TextField lastNameField = Util.createTextField("Last name", 650, 177, 145, 27);
            TextField insuranceIdField = Util.createTextField("Insurance ID", 650, 249, 145, 27);
            ...
            Button checkDriver = Util.createButton("Check", 827, 28, 60, 25);
            Button checkInsurance = Util.createButton("Check", 827, 250, 60, 25);
            Button AddButton = Util.createButton("Add", 497, 363, 110, 25);
            Button AddDriverButton = Util.createButton("Add Driver", 651, 363, 110, 25);
            Button drivingImageButton = Util.createButton("Driving image", 650, 299, 145, 25);

            drivingImageButton.setDisable(true);
            AddDriverButton.setDisable(true);
            checkInsurance.setDisable(true);
            AddButton.setDisable(true);

            TextField StringForDriverId = new TextField();
            TextField StringForInsuranceId = new TextField();
            Util.limitTextFieldLength(driverIdField, "Driver ID number is 9 Digit", 9);
            ...
            checkDriver.setOnAction(e -> {

                if (!Util.isEmptyField(driverIdField)) {
                    try {
                        resultSet = DBConnection.SearchForDriver(driverIdField.getText());

                        Util.alert.setTitle("Result");

                        if (resultSet.next()) {
                            Util.alert.setAlertType(Alert.AlertType.CONFIRMATION);
                            Util.alert.setContentText("Driver in DataBase");
                           ...
                           // String dob = resultSet.getString("dob");
                            //datePicker.setValue(LocalDate.of(Integer.parseInt(resultSet.getString("dob").split("/")[0]), Integer.parseInt(resultSet.getString("dob").split("/")[1]), Integer.parseInt(resultSet.getString("dob").split("/")[2])));
                            StringForDriverId.setText(resultSet.getString("ssn"));
                            datePicker.setValue(resultSet.getDate("dob").toLocalDate());

                            //datePicker.setValue(LocalDate.parse(dob));
                            datePicker.setDisable(false);

                            checkInsurance.setDisable(false);
                        } else {
                            Util.alert.setAlertType(Alert.AlertType.WARNING);
                            Util.alert.setContentText("Driver not in DataBase");
                            ...
                            datePicker.setValue(null);
                            datePicker.setDisable(false);
                            AddDriverButton.setDisable(false);
                            drivingImageButton.setDisable(false);
                        }
                        Util.alert.show();
                    } catch (SQLException ex) {
                    ...
                    Util.alert.setAlertType(Alert.AlertType.ERROR);
                    Util.alert.setTitle("Error");
                    Util.alert.setContentText("Insert the Id driver first");
                    Util.alert.show();
                }

            });

            checkInsurance.setOnAction(e -> {
                if (!Util.isEmptyField(insuranceIdField)) {
                    try {
                        resultSet = DBConnection.SearchForInsurance(insuranceIdField.getText());

                        Util.alert.setTitle("Result");

                        if (resultSet.next()) {
                            Util.alert.setAlertType(Alert.AlertType.CONFIRMATION);
                            Util.alert.setContentText("Insurance in DataBase");
                            AddButton.setDisable(false);
                            StringForInsuranceId.setText(resultSet.getString("ins_id"));
                            System.out.println(StringForInsuranceId.getText());
                        } else {
                            Util.alert.setAlertType(Alert.AlertType.WARNING);
                            Util.alert.setContentText("You cant add Driver for insurance not Exist in DataBase");
                    ...
                    Util.alert.setAlertType(Alert.AlertType.ERROR);
                    Util.alert.setTitle("Error");
                    Util.alert.setContentText("Insert the insurance id first");
                    Util.alert.show();
                }
            });
            AddDriverButton.setOnAction(e -> {
                if (!Util.isEmptyField(driverIdField) && !Util.isEmptyField(firstNameField) && !Util.isEmptyField(secondNameField) &&
                        !Util.isEmptyField(thirdNameField) && !Util.isEmptyField(lastNameField)) {
                    try {

                        DBConnection.insertDriver(driverIdField.getText(), firstNameField.getText(), secondNameField.getText()
                                , thirdNameField.getText(), lastNameField.getText(), datePicker, drivingImage);
                        Util.alert.setAlertType(Alert.AlertType.CONFIRMATION);
                        Util.alert.setTitle("Done");
                        Util.alert.setContentText("Driver added");
                        Util.alert.show();
                        AddButton.setDisable(false);
                        checkInsurance.setDisable(false);
                    } catch (SQLException ex) {
                    ...
                    Util.alert.setAlertType(Alert.AlertType.ERROR);
                    Util.alert.setTitle("Error");
                    Util.alert.setContentText("some Date are missing");
                    Util.alert.show();
                }
            });

            AddButton.setOnAction(e -> {
                if (!Util.isEmptyField(StringForDriverId) && !Util.isEmptyField(StringForInsuranceId)) {
                    try {
                        DBConnection.searchForDriverInsurance(StringForDriverId.getText(),
                                StringForInsuranceId.getText());
                        if (!resultSet.next()) {
                            DBConnection.InsertDriverInsurance(StringForDriverId.getText(), StringForInsuranceId.getText());
                            Util.alert.setAlertType(Alert.AlertType.CONFIRMATION);
                            Util.alert.setTitle("Done");
                            Util.alert.setContentText("Driver Added to Insurance");
                            Util.alert.show();
                        } else {
                            Util.alert.setAlertType(Alert.AlertType.ERROR);
                            Util.alert.setTitle("Error");
                            Util.alert.setContentText("Driver Already exist in DB for this insurance");
                            Util.alert.show();
                        }
                    } catch (SQLException ex) {
                        System.out.println(ex.getMessage());
                    ...
                    Util.alert.setAlertType(Alert.AlertType.ERROR);
                    Util.alert.setTitle("Error");
                    Util.alert.setContentText("the drive and insurance must be not Empty");
                    Util.alert.show();
                }
            });

            Util.thirdPane.getChildren().addAll(Util.stepTextArea, driverIdlabel, driverIdField, firstNameLabel, firstNameField, secondNameLabel,
                    secondNameField, thirdNameLabel, thirdNameField, lastNameLabel, lastNameField, insuranceIdLabel, checkDriver, AddDriverButton,
                    checkInsurance, insuranceIdField, dateOfBirthLabel, datePicker, AddButton, drivaingImageLabel, drivingImageButton);
            return Util.thirdPane;
        }
