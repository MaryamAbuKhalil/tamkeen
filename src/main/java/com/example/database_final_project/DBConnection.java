...
public class DBConnection {
    ...
    private static final String PASSWORD = "258852Meme";
    ...
    public static void insertDriver(String idNumber, String firstName, String secondName, String thirdName, String lastName, DatePicker dateOfBirth, byte[] drivingImage) throws SQLException {

        conn = DBConnection.getConnection();
        preparedStatement = conn.prepareStatement("insert into driver (ssn, first_name, second_name, third_name, fourth_name, dob) values(?,?,?,?,?,?)");
        preparedStatement.setString(1, idNumber);// (ssn, first_name, second_name, third_name, fourth_name) values(?,?,?,?,?)
        preparedStatement.setString(2, firstName);
        preparedStatement.setString(3, secondName);
        preparedStatement.setString(4, thirdName);
        preparedStatement.setString(5, lastName);
        preparedStatement.setDate(6, Date.valueOf(Util.formatterDate(dateOfBirth)));

        //preparedStatement.setDate(6, Util.formatterDate().);
        /*
        preparedStatement.setString(7, Arrays.toString(drivingImage));*/
    ...
    public static ResultSet SearchForDriver(String driverId) throws SQLException {
        conn = DBConnection.getConnection();
        preparedStatement = conn.prepareStatement("select * from driver where ssn=?");
        preparedStatement.setString(1, driverId);
        return preparedStatement.executeQuery();
    }

    public static ResultSet SearchForInsurance(String insuranceId) throws SQLException {
        conn = DBConnection.getConnection();
        preparedStatement = conn.prepareStatement("select * from insurance where ins_id=?");
        preparedStatement.setString(1, insuranceId);
        return preparedStatement.executeQuery();
    ...
    public static ResultSet searchForDriverInsurance(String driverId, String insuranceId) throws SQLException {
        conn = DBConnection.getConnection();
        preparedStatement = conn.prepareStatement("select * from driver_insurance where ins_id=? and driver_id=?");
        preparedStatement.setString(1, insuranceId);
    ...
    public static void InsertDriverInsurance(String driverId, String insuranceId) throws SQLException {
        conn = DBConnection.getConnection();
        preparedStatement = conn.prepareStatement("insert into driver_insurance values (?,?,?)");
        preparedStatement.setString(1, "100");
        preparedStatement.setString(2, driverId);
        preparedStatement.setString(3, insuranceId);
        preparedStatement.executeUpdate();
    }
